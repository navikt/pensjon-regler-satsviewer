...
import { eøsKonvensjonsLandSats } from "../../constants/Constants";
import { querySatsTabellByMiljøAndTypeAndAktiv } from "../../service/Queries";
import DefaultTable from "../DefaultTable";
import {useQueryClient, UseQueryResult} from '@tanstack/react-query';
import {EosKonvensjonsLandSatser, LandSats} from "../../model";

interface EØSKonvensjonslandTabellProps {
    environment: string;
    satstabell: string;
}
...
const EØSKonvensjonslandTabell: FC<EØSKonvensjonslandTabellProps> = ({ environment, satstabell }) => {
    ...
    const type = eøsKonvensjonsLandSats;
    ...
    const { data, isError, isLoading, isSuccess, isFetching } = querySatsTabellByMiljøAndTypeAndAktiv(environment, type, false, satstabell) as UseQueryResult<EosKonvensjonsLandSatser>;
                ...
                <Accordion.Content>
                    {isSuccess && data ?
                        <Table size="small" zebraStripes>
                            ...
                            <Table.Body>
                                {data.satser.map((rad: LandSats, key: number) => {
                                    return (
                                        <Table.Row key={key}>
