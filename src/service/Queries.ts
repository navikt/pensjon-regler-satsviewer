...
const fetchSatsByTabellByMiljøAndTypeAndAktivAndSatstabell = async (environment: string, type: string, aktiv: boolean, satstabell: string): Promise<SatserType> => {
    const response: FetchResponse = await fetch(`https://pensjon-regler-${environment}.ansatt.dev.nav.no/api/${type}?Aktiv=${aktiv}&Satstabell=${satstabell}`,
        {
            headers: {
...
export const fetchAktivSatsTabellByMiljø = async (environment: string): Promise<string> => {
    const response: FetchResponse = await fetch(`https://pensjon-regler-${environment}.ansatt.dev.nav.no/aktivTabell`,
        {
            headers: {
...
export const fetchAlleSatstabellerByMiljø = async (environment: string): Promise<Satser> => {
    const response: FetchResponse = await fetch(`https://pensjon-regler-${environment}.ansatt.dev.nav.no/alleSatstabeller`,
        {
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json'
